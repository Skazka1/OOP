#include "subj.h"
void PrintLegk(struct Legk* car) {
...
void InputLegk(struct Legk* car) {
...
void InputCar(struct Car* car) {
	...
	switch (car->carsType) {
	case Legk:
		InputLegk((struct Legk*)car);
		break;
	case Microbus:
		InputMicrobus((struct Microbus*)car);
		break;
	case Bus:
		InputBus((struct Bus*)car);
		break;
	case Gruz:
		InputGruz((struct Gruz*)car);
		break;
	case Spec:
		InputSpec((struct Spec*)car);
		break;
	}
}


struct Car* FindByNumber(struct List* list, char* number) {
	for (struct Item* i = list->head; i != NULL; i = i->next) {
		if (strcmp(((struct Car*)i)->GosNumber, number) == 0) {
			return (struct Car*)i;
		}
	}
}
struct List* FindByOwner(struct List* list, char* owner){
	...
	result->tail = NULL;
	for (struct Item* i = list->head; i != NULL; i = i->next) {
		if (strcmp(((struct Car*)i)->Owner, owner) == 0) {
			Add(result, i);
		}
	}
	return result;
...
struct List* FindByColor(struct List* list, char* color) {
	...
	result->tail = NULL;
	for (struct Item* i = list->head; i != NULL; i = i->next) {
		if (strcmp(((struct Car*)i)->Color, color) == 0) {
			Add(result, i);
		}
	}
	return result;
...
struct Car* Create(enum CarsType createTypeCar){
	struct Car* item = NULL;
	switch (createTypeCar) {
	case Legk:
		item = (struct Car*)calloc(1, sizeof(struct Legk));
		break;
	case Microbus:
		item = (struct Car*)calloc(1, sizeof(struct Microbus));
		break;
	case Bus:
		item = (struct Car*)calloc(1, sizeof(struct Bus));
		break;
	case Gruz:
		item = (struct Car*)calloc(1, sizeof(struct Gruz));
		break;
	case Spec:
		item = (struct Car*)calloc(1, sizeof(struct Spec));
		break;
	}
	if (item != NULL)
...
void SortByCapacity(struct List* list, int reverse) {
	...
	for (i = 0; i < len; i++) {
		for (j = 0; j < len - 1; j++) {
			p1 = (struct Car*)GetItem(list, j);
			p2 = (struct Car*)GetItem(list, j + 1);

			int shouldSwap = 0;
			if (reverse) {

				shouldSwap = p1->Capacity < p2->Capacity;
			}
			else {
